// Dependencies: npm install emoji-regex

var redacter = (function (root) {
    ...
    function wrapText(node, blocks, tagName = 'span', className = 'emoji-wrapped') {
    ...
    function wrapAllEmoji(selector) {
        ...
        return nodeList.map(wrapNodeEmoji).reduce((m, a) => m.concat(a), []);
    }

    function redactText(selector) {
        ...
        var dims = wrapped.map(node => node.getBoundingClientRect());
